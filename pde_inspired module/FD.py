...
from aTEAM.nn.functionals.utils import periodicpad
...
def _torch_setter_by_index(t,i,v):  # t:moment[i](shape: [5,5]), i:o([0,0]), 1
    for j in i[:-1]:
        t = t[j]
    t[i[-1]] = v
...
class MomentBank(nn.Module):
    ...
    def __init__(self, dim, kernel_size, max_order, dx=1.0, constraint='moment'):
        ...
        if isinstance(kernel_size, int):
            kernel_size = [kernel_size, ] * self.dim  # [5, 5]
        assert min(kernel_size) > max_order
        self.m2k = MK.M2K(kernel_size)  # [5, 5]
        self._kernel_size = kernel_size.copy()
        self._max_order = max_order
        if not iterable(dx):
            dx = [dx, ] * dim  # 把 dx 变成多个维度数的偏导
        self._dx = dx.copy()
        ...
        for a in self._order_bank:
            N += len(a)
        moment = torch.zeros(*([N, ] + kernel_size))
        index = zeros([m+1, ] * dim, dtype=np.int32)
    ...
    def kernel(self):
        ...
        kernel = Variable(self.m2k(self.moment))
        size = kernel.size()
...
class _FDNd(nn.Module):
    ...
    Arguments for class:`_FDNd`:
        ...
        boundary (string): 'Dirichlet' or 'Periodic' Dirichlet边界条件 或者 Periodic边界条件 是两种不同的边界条件
